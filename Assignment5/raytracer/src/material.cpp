...
Vec3f PhongMaterial::Shade(const Ray &ray, const Hit &hit, const Vec3f &dirToLight, const Vec3f &lightColor,
                           const bool shade_back) const {
    Vec3f normal = hit.getNormal();
    if (normal.Dot3(ray.getDirection()) > 0 && shade_back)normal.Negate();
    ...
    Vec3f diffuse = diffuseColor * lightColor * max(normal.Dot3(dirToLight), 0.0f);
